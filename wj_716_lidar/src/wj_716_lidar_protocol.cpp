...
//    scan.ranges.resize(config_.resize);
    ...
    int resizeNum;
    cout << "resizeNum:" <<config_.resize<<endl;
    return true;
  }
   ...
   bool wj_716_lidar_protocol::dataProcess(unsigned char *data, const int reclen)
   {
     ...
     while(m_sdata.m_u32out < m_sdata.m_u32in)
     {
       ...
       else if (m_sdata.m_acdata[m_sdata.m_u32out] == 0xff && m_sdata.m_acdata[m_sdata.m_u32out+1] == 0xaa)
       {
        ...
        m_sdata.m_u32out += l_u32reallen;
        if(m_sdata.m_acdata[22]==0x05 && m_sdata.m_acdata[23]==0x01 )
        {

         unsigned sxxx  = (m_sdata.m_acdata[89] << 8)  |
                          (m_sdata.m_acdata[88] << 0);
         unsigned cxxx  = (m_sdata.m_acdata[95] << 8)  |
                          (m_sdata.m_acdata[94] << 0);

          cout<<"sxxx: "<<sxxx<<"     cxxx: "<<cxxx<<endl;
        }
       }
       else
       {
